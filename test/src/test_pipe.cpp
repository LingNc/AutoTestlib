...
TestSuite create_pipe_tests() {
    ...
    // 测试基本创建和关闭
    suite.add_test("创建和关闭", []() -> std::string {
        pc::Pipe pipe;
    ...
    // 测试读写基本功能
    suite.add_test("基本读写", []() -> std::string {
        pc::Pipe pipe;
    ...
    // 测试阻塞和非阻塞模式
    suite.add_test("阻塞和非阻塞模式", []() -> std::string {
        pc::Pipe pipe;
    ...
    // 测试empty函数
    suite.add_test("测试empty", []() -> std::string {
        pc::Pipe pipe;
    ...
    // 测试句柄操作
    suite.add_test("句柄操作", []() -> std::string {
        pc::Pipe pipe;
    ...
    // 测试缓冲区大小设置
    suite.add_test("缓冲区大小设置", []() -> std::string {
        pc::Pipe pipe;
    ...
    // 测试非阻塞读取超时
    suite.add_test("非阻塞读取超时", []() -> std::string {
        pc::Pipe pipe;
        pipe.set_blocked(false);
